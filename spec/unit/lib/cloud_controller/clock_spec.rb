    ...
    describe "#start" do
      ...
      let(:config) do
        {
          ...
          pending_packages: { frequency: 1.minute, expiration: 5.minutes },
        }
      end
      ...
      describe "pending_packages.cleanup.job" do
        it "schedules a PendingPackagesCleanup job to run every N minutes" do
          expect(Clockwork).to have_received(:every).with(1.minute, "pending_packages.cleanup.job", expiration: 5.minutes)
