  ...
  class Membership
    ...
    def space_guids
      ...
      if @user.admin?
        ...
        spaces_from_orgs = Space.join(:organizations, organizations__id: :spaces__organization_id).where(organizations__guid: org_guids).select(:spaces__guid)
        dataset = spaces + spaces_from_orgs
      end
