module VCAP::CloudController
  class ProcfileParse
    ...
    def process_procfile(app, procfile=nil)
      @logger.info('proccess_procfile', guid: app.guid)

      if procfile
        @logger.debug('using the given procfile', guid: app.guid)

        procfile_hash = Procfile.load(procfile)
        app.db.transaction do
          app.lock!
          converge_on_procfile(app, procfile_hash)
          app.update(procfile: procfile)
        end
      elsif app.desired_droplet && app.desired_droplet.procfile
