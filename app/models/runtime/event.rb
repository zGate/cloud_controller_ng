  ...
  class Event < Sequel::Model
    ...
    def self.user_visible(user, admin_override=false)
      if admin_override
        ...
        where(space: user.spaces_dataset).union(where(space: user.audited_spaces_dataset)).union(where(organization_guid: user.audited_organizations_dataset.map(&:guid)))
      else
